...
class VendorReports extends Component
{
    ...
    public function render()
    {
        ...
        if ($this->datefilter && $this->vendor_id && $this->search) {
            ...
            $statements = VendorStatement::with('vendor')->whereHas('vendor', function ($query) {
            ...
            $start = date('Y-m-d', strtotime($explode[0])) . ' 00:00:00';
            $end = date('Y-m-d', strtotime($explode[1])) . ' 23:59:59';
            $statements = VendorStatement::with('vendor')->whereBetween('created_at', [$start, $end])->orderBy('id', 'desc')->paginate($this->rows);
        } elseif ($this->vendor_id) {
            $statements = VendorStatement::with('vendor')->whereHas('vendor', function ($query) {
                $query->where('id', $this->vendor_id);
            })->orderBy('id', 'desc')->paginate($this->rows);
        } elseif ($this->search) {
            $statements = VendorStatement::with('vendor')->where('source', 'like', '%' . $this->search . '%')->orWhere('amount', 'like', '%' . $this->search . '%')->orWhere('pay_via', 'like', '%' . $this->search . '%')->orWhere('created_at', 'like', '%' . $this->search . '%')->orderBy('id', 'desc')->paginate($this->rows);
        } else {
